...
hue=(-level "0%,100%,0.6")
effect=(-scale 10% -blur 0x2 -scale 1000%)
...
set -o pipefail
trap 'echo "--- Cleaning up temporary file $image ---"; rm -f "$image"' EXIT
echo "--- Parsing options with getopt ---"
...
if "$shot_custom" && [[ $# -gt 0 ]]; then
    echo "--- Using custom screenshot command: $@ ---"
    shot=("$@");
fi

echo "--- Taking screenshot ---"
command -- "${shot[@]}" "$image"
echo "--- Screenshot saved to $image ---"

value="60" #brightness value to compare to

# Get brightness value
brightness=$(magick "$image" -colorspace HSL -channel g -separate +channel -format "%[fx:mean*100]" info:)
echo "--- Image brightness: $brightness ---"

if [ $(echo "$brightness > $value" | bc) -eq 1 ]; then
    echo "--- Image is bright, applying dark text ---"
    bw="black"
    icon="/home/shui/.config/i3/scripts/icons/lockdark.png"
    ...
    echo "--- Image is dark, applying light text ---"
    bw="white"
    icon="/home/shui/.config/i3/scripts/icons/lock.png"
